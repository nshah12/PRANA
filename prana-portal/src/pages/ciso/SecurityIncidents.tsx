    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className={`rounded-xl p-4 border ${p0Open ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
          ...
          <p className="text-xs text-slate-400 mt-1">{tUi('CISO_SEC_INC_P0_SLA')}</p>
        </div>
        <div className={`rounded-xl p-4 border ${p1Open ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-200'}`}>
          ...
          <p className="text-xs text-slate-400 mt-1">{tUi('CISO_SEC_INC_P1_SLA')}</p>
        </div>
        <div className="rounded-xl p-4 border bg-white border-slate-200">
          ...
          <p className="text-xs text-slate-400 mt-1">{tUi('CISO_SEC_INC_ALL_SEV_NOTE')}</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center py-16 text-slate-400">
          <p className="text-sm">{tUi('CISO_SEC_INC_LOAD_FAILED')}</p>
        ...
        <div className="flex flex-col items-center py-16 text-slate-400">
          ...
          <p className="font-medium text-slate-600">{tUi('CISO_SEC_INC_EMPTY')}</p>
        </div>
      ) : (
        <div className="space-y-3">
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-400">
                      <span className="flex items-center gap-1">
                        <Clock size={10} /> {tUi('CISO_SEC_INC_CREATED_PREFIX')} {new Date(inc.created_at).toLocaleString('en-IN')}
                      </span>
                      {inc.resolved_at && (
                        <span className="text-emerald-600">
                          ✓ {tUi('CISO_SEC_INC_RESOLVED_PREFIX')} {new Date(inc.resolved_at).toLocaleString('en-IN')}
                        </span>
                      )}
                      {inc.escalated_at && (
