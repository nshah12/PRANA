...
export function EmployeeTotpReset() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await api.post('/admin/employees/reset-totp', form)
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div>
        <h1 className="text-xl font-semibold text-slate-800">{tUi('PA_RESET_TOTP_TITLE')}</h1>
      ...
      <form onSubmit={submit} className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 space-y-4">
        <h2 className="font-medium text-slate-800 flex items-center gap-2">
          <Zap size={14} className="text-red-500"/> {tUi('PA_RESET_TOTP_SECTION_TITLE')}
        </h2>

        <div className="space-y-1">
          <label htmlFor="pa-reset-totp-identifier"
                 className="text-xs font-medium text-slate-500 uppercase tracking-wide">
            {tUi('PA_RESET_TOTP_IDENTIFIER_LABEL')}
          </label>
          <input
            id="pa-reset-totp-identifier"
        ...
        <div className="space-y-1">
          <label htmlFor="pa-reset-totp-reason"
                 className="text-xs font-medium text-slate-500 uppercase tracking-wide">
            {tUi('PA_RESET_TOTP_REASON_LABEL')} *
          </label>
          <input
            id="pa-reset-totp-reason"
            value={form.reason}
