/**
 * PA — Platform Document Field Manifest editor.
 *
 * Same concept as oa/DocumentFields.tsx but manages the platform-DEFAULT
 * manifest (tenant_id IS NULL) that every tenant without their own override
 * inherits. Marking a field "safe" here only affects tenants that haven't
 * created their own manifest override for that doc type.
 *
 * API: GET /admin/manifests            → { items: [manifest, ...] } (platform defaults)
 *      PUT /admin/manifests/{doc_type} → full ManifestUpsertRequest body
 */
...
export function PlatformDocumentFields() {
  const qc = useQueryClient()

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['pa-manifests'],
    queryFn: () => api.get('/admin/manifests').then(r => r.data),
  })
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return api.put(`/admin/manifests/${current.doc_type}`, {
    ...
    onSuccess: () => {
      setSaved(true)
      setErrorMsg(null)
      qc.invalidateQueries({ queryKey: ['pa-manifests'] })
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center py-16 text-slate-400">
        <p className="text-sm">{tUi('PA_DOC_FIELDS_LOAD_FAILED')}</p>
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center py-16 text-slate-400">
        ...
        <p className="font-medium text-slate-600">{tUi('PA_DOC_FIELDS_EMPTY')}</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-800 flex items-center gap-2">
          <ShieldCheck size={20} className="text-indigo-500" /> {tUi('PA_DOC_FIELDS_TITLE')}
        </h1>
        <p className="text-sm text-slate-500 mt-0.5">{tUi('PA_DOC_FIELDS_SUB')}</p>
      </div>
        ...
        {current && (
          <div className="flex-1 bg-white rounded-xl border border-slate-100 shadow-sm p-6 space-y-4">
            <div className="flex gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5">
              ...
              <p className="text-xs text-slate-500 leading-4">{tUi('PA_DOC_FIELDS_SAFE_EXPLAINER')}</p>
            </div>
            ...
            <div className="divide-y divide-slate-100">
              {fields.map(field => {
                ...
                return (
                  <label
                    key={field}
                    className="flex items-center justify-between py-3 select-none cursor-pointer"
              ...
              {fields.length === 0 && (
                <p className="text-xs text-slate-400 py-3">{tUi('PA_DOC_FIELDS_NO_FIELDS')}</p>
              )}
            </div>
              ...
              >
                {saved
                  ...
                  : saveMutation.isPending
                    ...
                    : <><Save size={14} /> {tUi('PA_DOC_FIELDS_SAVE_BTN')}</>
                }
              </button>
            </div>
          </div>
