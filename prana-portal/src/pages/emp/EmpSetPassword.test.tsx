...
import { EmpSetPassword } from './EmpSetPassword'
...
function renderPage() {
  return render(<MemoryRouter><EmpSetPassword /></MemoryRouter>)
}
...
describe('EmpSetPassword', () => {
  it('shows a checking state, then verifies the token and shows the form with the account email', async () => {
    vi.mocked(api.post).mockResolvedValueOnce({ data: { valid: true, email: 'newhire@acme.com' } })
    renderPage()

    expect(screen.getByText(/checking/i)).toBeInTheDocument()
    expect(await screen.findByText(/newhire@acme\.com/)).toBeInTheDocument()
    expect(api.post).toHaveBeenCalledWith('/auth/employee/password-setup/verify', { token: 'real-setup-token' })
  })
  ...
  it('on success: calls password-setup with the URL token and shows the done state', async () => {
    vi.mocked(api.post)
      .mockResolvedValueOnce({ data: { valid: true, email: 'newhire@acme.com' } })   // verify
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith('/auth/employee/password-setup', {
    ...
    expect(mockNavigate).toHaveBeenCalledWith('/emp/login')
  })

  it('falls back to the invalid-link state if the token expires between verify and submit', async () => {
    vi.mocked(api.post)
      .mockResolvedValueOnce({ data: { valid: true, email: 'newhire@acme.com' } })
