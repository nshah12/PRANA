...
async def verify_totp(body: OATOTPIn, request: Request, response: Response, db: DbConn):
    ...
    if not valid:
        # Atomic increment (H5) — single UPDATE ... RETURNING avoids the read-modify-write
        # race where concurrent attempts each read the same count and under-count the lockout.
        new_count = await db.fetchval(
            "UPDATE oa_user SET failed_totp_count = failed_totp_count + 1 "
    ...
    if not await consume_totp_code(redis_client, "oa_user", oa_user_id, body.code):
        await _log_attempt(db, "oa_user", oa_user_id, "TOTP", "FAILED", "TOTP_REPLAY", ip)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=PranaError.INVALID_TOTP)
