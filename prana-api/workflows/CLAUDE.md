...
**Fixed 2026-07-22:** a full grep for every "Pattern 3" label in `workflows/*.py` found that only `SystemHealthWorkflow`, `AuditIntegrityVerificationWorkflow`, and `ErrorThresholdEvaluationWorkflow` had an `ensure_*_schedule()` call wired into `main.py`'s startup. `PlatformSummaryWorkflow`, `DigestWorkflow` (weekly + monthly), `KMSHealthCheckWorkflow`, `StorageQuotaCheckWorkflow`, and `AuditArchivalWorkflow` had no schedule-registration code anywhere despite being registered on their task queues in `worker.py`; `ClamAVUpdateWorkflow` and `StagingCleanupWorkflow` weren't even registered on a worker queue at all (0 workers could have run them if started manually). `StatutoryComplianceWorkflow` is per-tenant (`statutory-compliance-{tenant_id}`) and now gets an `ensure_statutory_compliance_schedules()` pass at startup plus a real trigger on `TenantConsumer`'s `TENANT_ACTIVATED` (previously a no-op). All now register real Schedules and are verified with tests that actually invoke the function against the real `temporalio` SDK shapes, not source inspection. `RetentionWorkflow` was checked and needs no fix — `employee_lifecycle.py`'s `start_retention_workflow` already starts it correctly on `EmployeeExitWorkflow`.
