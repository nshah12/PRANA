"""
Tests for routers/pa_admin.py — Portal Admin platform overview & health.

Split 2026-08-10 alongside the routers/pa_admin.py split into 4 domain files
(this one, test_pa_admin_accounts.py, test_pa_admin_security.py,
test_pa_admin_comms.py) — all still exercise the same /admin/* HTTP surface
via the `client` fixture, so no behavior changed, only file organization.

Covers: auth guard, tenant activation (via tenants.py), emergency override
Kafka event, cross-tenant capability, reject/retry-verification, rate limits,
duplicate-route guards, meta-dashboard, secops overview.
"""
...
def _secops_fetchval_side_effect(sql, *args):
    s = sql.lower()
    if "account_status_event" in s:
        return 6
    if "ip_country" in s:
        return 3
    if "anomaly_event" in s:
        return 9
    if "login_attempt_log" in s:
        return 214
    return 0
...
def _secops_fetch_side_effect(sql, *args):
    s = sql.lower()
...
@pytest.mark.asyncio
async def test_secops_requires_portal_admin_role(client, mock_db):
    """OA-Admin must NOT be able to read platform-wide secops stats — PA_ONLY."""
    _set_oa_auth(client)

    resp = await client.get("/admin/secops", headers=AUTH_HEADER)

    assert resp.status_code == 403


@pytest.mark.asyncio
async def test_secops_returns_platform_wide_stats(client, mock_db):
    _set_pa_auth(client)
    mock_db.fetchval = AsyncMock(side_effect=_secops_fetchval_side_effect)
    mock_db.fetch = AsyncMock(side_effect=_secops_fetch_side_effect)

    resp = await client.get("/admin/secops", headers=AUTH_HEADER)

    assert resp.status_code == 200
    body = resp.json()
    assert body["active_threats"] == 9
    assert body["locked_accounts"] == 6
    assert body["auth_events_1h"] == 214
    assert body["foreign_ips_24h"] == 3


@pytest.mark.asyncio
async def test_secops_tenant_posture_table_shape(client, mock_db):
    """Frontend (SecOpsDashboard.tsx) reads data.tenants[].{tenant_id, tenant_name,
    posture, locked_count, anomaly_count, last_threat_at} — response must match exactly.
    """
    _set_pa_auth(client)
    mock_db.fetchval = AsyncMock(side_effect=_secops_fetchval_side_effect)
    mock_db.fetch = AsyncMock(side_effect=_secops_fetch_side_effect)
    ...
    assert row["tenant_name"] == "NPCI"
    assert row["posture"] == "RED"     # threats_24h=7 > 5
    assert row["locked_count"] == 3
    assert row["anomaly_count"] == 2
    assert row["last_threat_at"] is None


@pytest.mark.asyncio
async def test_secops_live_alert_feed_shape(client, mock_db):
    """Frontend reads data.alerts[].{severity, description, tenant_name, occurred_at}."""
    _set_pa_auth(client)
    mock_db.fetchval = AsyncMock(side_effect=_secops_fetchval_side_effect)
    mock_db.fetch = AsyncMock(side_effect=_secops_fetch_side_effect)
    ...
    assert len(alerts) == 1
    alert = alerts[0]
    assert alert["severity"] == "HIGH"          # P0 -> HIGH
    assert alert["description"] == "BRUTE_FORCE"
    assert alert["tenant_name"] == "NPCI"
